...
class DatabaseConnection {
  ...
  async init() {
    ...
    try {
      // PERBAIKAN: Langsung buat pool koneksi menggunakan dbConfig bawaan Railway.
      // Kita lewati proses membuat database root karena database 'railway' sudah di-provisioning otomatis.
      ...
      // Tes koneksi sederhana untuk memastikan kredensial cloud sudah benar
      const connectionTest = await this.pool.getConnection();
      console.log('[DB Engine] Koneksi ke server MySQL berhasil mapan.');
      connectionTest.release();

      // 4. Buat Tabel Soal (Jika belum ada)
      ...
      if (rows[0].total === 0) {
        ...
        console.log('[DB Engine] Sukses melakukan seeding 3 soal bawaan.');
      } else {
        console.log('[DB Engine] Data soal terdeteksi ready. Seeding dilewati.');
      }

    } catch (err) {
      // Amankan error agar tidak merusak siklus hidup main process server Node
      console.error('[MySQL Error] Terjadi kendala internal struktur objek database:', err.message);
    }
  }
  ...
  async getQuestions() {
    try {
      // PERBAIKAN MENDESAK: Beri batas limit rekursif agar jika DB error, server tidak gantung/memory leak
      if (!this.pool) {
        console.log('[DB Engine Warning] Menunggu pool koneksi siap... (Menggunakan cadangan jika gagal)');
        return null; 
      }
